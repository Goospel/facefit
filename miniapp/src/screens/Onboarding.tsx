  ...
  return (
    <main style={ui.pageFull}>
      ...
      <div style={{ display: 'grid', gap: 20, marginTop: 8 }}>
        ...
        />
        <div>
          <b style={{ fontSize: 15 }}>사진은 이 기기에만</b>
          {/* ⚠️ 검수 통과 문장이라 **한 줄 그대로** 둔다 — 다른 문장에 섞어 넣으면
              화면마다 미묘하게 달라지고, 그때부터 「어느 쪽이 사실인가」가 된다.
              v3에서 서버가 생겼지만 **이 문장은 글자 하나 안 바뀐다**(설계 §3-8의 성과). */}
          <p style={{ ...ui.sub, margin: '4px 0 0' }}>{LOCAL_ONLY}</p>
          {/* ⚠️ 「계정도 서버도 없어요」였다 — v3에서 서버가 생겨 **거짓이 됐다.** 로그인이
              없다는 사실은 그대로라 그 절반만 남긴다. 서버가 생긴 것과 계정이 생긴 것은 다르다. */}
          <p style={{ ...ui.sub, margin: '4px 0 0' }}>
            카메라는 얼굴 사진을 찍는 데에만 쓰고, 로그인도 계정도 없어요.
          </p>
          {/* ⚠️ v2-2에서 「런타임 네트워크 호출 0」이 처음 깨졌다(설계 §3-5). 뒤에 붙어 있던
              「나가는 건 검색어뿐이에요」는 백업이 생긴 지금 **거짓이라 걷었다** — 문구를
              더하는 것보다 **안 지운 옛 문장**이 더 위험하다(새 문장과 정면으로 모순된다). */}
          <p style={{ ...ui.sub, margin: '4px 0 0' }}>
            제품을 검색할 때만 식약처 공공 데이터를 조회해요.
          </p>
          {/* v3 신설(설계 §3-8). 검수자가 번들에서 서버 호출을 발견하고 묻기 전에 먼저 말한다. */}
          <p style={{ ...ui.sub, margin: '4px 0 0' }}>
            기록 백업을 켜면 제품과 관찰 기록이 서버에 저장돼요 — 기기를 바꿔도 이어 보기
            위해서예요. <b>얼굴 사진은 올라가지 않아요.</b> 백업은 「오늘」 탭에서 언제든 켜고
            끌 수 있고, 끄면 서버에 있던 기록도 함께 지워져요.
          </p>
        </div>
      </div>
