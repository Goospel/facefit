...
- **토스 로그인은 필요 없다(실측).** 발송 API 헤더 `x-anon-key`가 「미니앱 SDK의 `User.getAnonymousKey` 함수로 발급받을 수
  ...
  ✅ **2026-09-03 16:45 KST 실수신으로 확정됐다**(§2-2) — 진짜 익명 키로 발송이 성공했다. ⓐ′(토스 로그인) 폴백은 쓰지 않는다.
...
- ✅ **결론(2026-09-03 16:45 KST) — 전자가 맞다.** 실기기 예약이 3시간 뒤 실제로 도착했고, 서버 행은 발송 성공으로 삭제됐다.
  `x-anon-key`는 `send-message`에서 **수용된다**. 후자(`hash-key.md`)의 「토스 서버 API 호출용 키가 아니다」는 이 API에는 해당하지 않는다.
  ⚠️ 가르는 실험은 **실수신 하나뿐이었다** — `anon-key/verify` 스모크는 가짜 키든 진짜 키든 같은 4010을 돌려줘 판별력이 없다(절차 4).
...
| ⓐ′ 위와 같되 토스 로그인 userKey | ✅ | ✅ | **필요** | ⓐ + 콘솔 토스로그인 약관 동의(`toss_login_update_terms`)·clientId + 클라 로그인 화면 + 서버 OAuth 코드 교환(`사용자 정보 받기` API) + userKey 저장 + **개인정보 문구 전면 개정**(수집 0 원칙 포기) — 대략 ⓐ의 2~3배, 검수 리스크 별도 | ❌ **쓰지 않는다 — §2-2 가정이 2026-09-03 16:45 실수신으로 확정됐다**(진짜 익명 키 발송 성공). 폴백으로 검토했던 이력만 남긴다 |
...
```
[미예약] --썼어요--> [묻는 중] --알림 받기--> [예약됨: due = now+3h] --(서버 발송)--> [알림 뒤 · 승인 대기]
   ^                  |  괜찮아요              |  그만 받기                            |  썼어요 → [묻는 중]
   |                  v                       v                                     |  오늘은 그만 / 무응답 → 끝
   +------------------+-----------------------+-------------------------------------+
...
⚠️ **추기(2026-09-04 · 사용자 지적 · 이 개편의 출발점)**: 원안은 알약 하나가 「썼어요 · 다음 알림」이라 **탭 한 번이 두 가지 일**을 했다 — 기름종이를 썼다고 표시하는 것과 알림을 받겠다는 것. 그래서 ⓐ 사용을 알리려던 사람에게 **동의 시트부터** 들이밀었고 ⓑ 눌러도 무슨 일이 일어났는지가 흐렸다. **[묻는 중]을 끼워 둘로 나눈다** — 1단계 「썼어요」는 화면만 옮기고(서버·동의 시트 **0회**), 동의는 실제로 알림을 요청하는 2단계에서만 묻는다. `[예약됨]`에서 또 썼을 때도 **같은 두 걸음**을 걷는다(1탭 지름길을 일부러 안 둔다 — 규칙이 하나라야 외울 것이 없다).

⚠️ **[묻는 중]은 저장하지 않는다**(세션 한정). 「방금 썼다」는 그 순간의 맥락이라, 앱을 껐다 켜서까지 질문이 남아 있으면 언제 무엇에 답하는지 모르게 된다 — 다시 열면 1단계부터가 맞다. 저장되는 것은 여전히 `oilNextAt` **하나뿐**이다.
...
|---|---|---|---|
| 미예약 | 「쓰고 나서 눌러 주세요」(text-sub) | 알약 「썼어요」 | — |
| 묻는 중(세션 한정) | 「3시간 뒤에 알려드릴까요?」(blue-dark 600) | 알약 「알림 받기」 | ghost 「괜찮아요」 |
| 예약됨 | 「**15:20**에 알려드릴게요」(blue-dark 600) | check + 「예약됨」 | ghost 「그만 받기」 |
| 승인 대기(`nextAt ≤ now`) | 「알림을 보냈어요 · 또 썼으면 눌러 주세요」(blue-dark 600) | 알약 「썼어요」 | ghost 「오늘은 그만」 |
...
- `aria-label`: 행동 버튼 「기름종이 썼어요」(미예약·승인 대기 — **하는 일이 같아 같은 이름**) / 「기름종이 또 썼어요」(예약 중) / 「3시간 뒤 알림 받기」(묻는 중) · 취소 줄 「알림 없이 넘어가기」(묻는 중) / 「기름종이 알림 그만 받기」. `role="switch"`는 **안 쓴다** — 켜짐의 반대가 「꺼짐」이 아니라 「오늘은 없음」이라 스위치 은유가 틀린다.
- ⚠️ **예약이 확정된 순간에만 짧은 움직임**(280ms scale pop)을 오른쪽 check 표식에 준다 — 알약이 표식으로 바뀌는 것만으로는 눈을 떼고 있던 사람이 놓친다(「눌렀는지 모르겠다」가 이 개편의 출발점). **1회성 신호**라 그냥 켜 둔 화면은 조용하다. `@keyframes`가 아니라 **Web Animations API**인 것은 이 앱이 `className`을 안 쓰기 때문이고(스타일 전부 인라인), `prefers-reduced-motion: reduce`면 건너뛴다(문구·표식은 그대로 바뀐다). `animate`가 없는 환경에서도 카드가 죽지 않게 `?.`로 부른다 — 움직임은 있으면 좋은 것이지 기능이 아니다.
- ⚠️ **추기(2026-09-04 · 사용자 결정 · T-021)**: 원안은 예약됨 상태의 행을 **누를 수 없게**(`div`) 두었다 — 「예약된 것을 또 누르라고 하지 않는다」. **뒤집는다.** 기름종이는 하루에 여러 번 쓰는 물건이라 3시간을 다 기다리지 않고 또 쓰는 것이 정상인데, 그때 누를 표면이 없으면 「그만 받기 → 다시 누르기」 2단계를 거쳐야 다음 알림을 옮길 수 있었다. 서버는 원래 키당 한 줄을 덮어쓰므로(§3-4) **막고 있던 것은 화면뿐이다.** 「또 누르라」로 읽히는 문제는 오른쪽이 아니라 왼쪽 문구가 푼다(「또 썼으면 눌러요」) — 오른쪽은 check 표식 그대로 사실만 말한다.
- ⚠️⚠️ **상태는 스스로 흘러야 한다(T-021)**: `oilState(nextAt, new Date())`는 **그리는 순간에만** 계산되는데 시간의 경과는 리렌더를 안 일으킨다 — 알림이 도착해도 화면이 「예약됨」에 고착해 다음 알림을 켤 경로가 사라졌다(라이브 결함). 계기를 **둘** 둔다: ① 예약 시각 도달 `setTimeout`(열어 둔 채 넘길 때) ② `visibilitychange`(백그라운드에서 타이머가 스로틀될 때). 어느 하나로는 구멍이 남는다.
...
1. ~~**`x-anon-key`가 `send-message`에서 실제로 수용되는가**(§2-2 모순)~~ — ✅ **해소(2026-09-03 16:45 실수신)**. 예상과 달리 절차 4의 verify 스모크는 판별력이 없었고(가짜 키와 진짜 키가 같은 4010), 최종 판정은 **실기기 진짜 키의 3시간 뒤 실수신** 하나였다.
