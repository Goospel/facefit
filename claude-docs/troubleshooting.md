...
- [T-021](troubleshooting/T-021.md) · 렌더에서 읽은 `new Date()`는 흐르지 않는다 — 알림이 도착해도 카드가 「예약됨」에 얼어붙어 다음 알림을 켤 방법이 사라졌다
- [T-020](troubleshooting/T-020.md) · 스모크가 성공에도 실패에도 같은 응답을 주면 계측기가 아니다 — 4010 하나로 「mTLS 확정」까지만 갔고 정작 최대 가정은 못 가렸다 **→ 글로벌 CLAUDE.md 원칙(2026-09-03) — 4개 프로젝트 7건 재발이 확인돼 「번들 문자열 검사」 절을 일반 원칙으로 넓혔다**
